...
class SolcastApi:
    ...
    async def sites_weather(self):
        ...
        try:
            if len(self._sites) > 0:
                ...
                _LOGGER.debug("SOLCAST - get rooftop weather byline from solcast")
                async with async_timeout.timeout(60):
                    resp: ClientResponse = await self.aiohttp_session.get(
                        url="https://api.solcast.com.au/json/reply/GetRooftopSiteSparklines",
                        params=params,
                        ssl=False,
                    )
                    resp_json = await resp.json(content_type=None)
                ...
                if status == 200:
                    ...
                    self._weather = d.get("forecast_descriptor", None).get(
                        "description", None
                    )
                    _LOGGER.debug(
                        f"SOLCAST - rooftop weather description: {self._weather}"
                    )
                else:
                    raise Exception(
                        f"SOLCAST - sites_weather: gathering rooftop weather description failed. request returned Status code: {status} - Responce: {resp_json}."
                    )

        except json.decoder.JSONDecodeError:
            _LOGGER.error(
                "SOLCAST - sites_weather JSONDecodeError.. The rooftop weather description from Solcast is unknown, Solcast site could be having problems"
            )
        except ConnectionRefusedError as err:
            ...
            _LOGGER.error(
                "SOLCAST - sites_weather Connection Error - Timed out connection to solcast server"
            )
        except Exception:
    ...
    def get_peak_w_time_day(self, dayincrement) -> dt:
        ...
        try:
            ...
            return max((z for z in g), key=lambda x: x[self._use_data_field])[
                "period_start"
            ]
        except Exception:
            return None
    ...
    def get_total_kwh_forecast_day(self, dayincrement) -> float:
        ...
        for idx in range(1, len(self._data_forecasts)):
            ...
            ret += (
                (prev[self._use_data_field] + curr[self._use_data_field])
                / 2
                * diff_hours
            )
            needed_delta -= delta
